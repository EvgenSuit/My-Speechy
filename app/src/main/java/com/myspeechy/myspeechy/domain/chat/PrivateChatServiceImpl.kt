...
class PrivateChatServiceImpl(
    ...
    private var isOtherUserMemberOfChatListener: ValueEventListener? = null
    ...
    suspend fun joinChat(chatId: String, otherUser: Boolean) {
        val userIds = chatId.split("_")
        userIds.forEach { id ->
            if (id == userId || otherUser) {
                chatsRef.child(id)
                    .child(chatId)
    ...
    fun listenIfIsOtherUserMemberOfChat(userId: String, chatId: String,
                               onCancelled: (String) -> Unit,
                               onReceived: (Boolean) -> Unit, remove: Boolean) {
        val ref = usersRef.child(userId)
            .child("private_chats")

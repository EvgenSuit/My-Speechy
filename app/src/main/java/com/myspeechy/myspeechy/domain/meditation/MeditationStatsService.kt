package com.myspeechy.myspeechy.domain.meditation

import com.myspeechy.myspeechy.domain.MeditationTimeExceededException
...
class MeditationStatsServiceImpl(
    ...
    suspend fun updateStats(date: String, seconds: Int) {
        ...
        if (userId != null && seconds > 0) {
            val totalMeditationTime = (firestoreRef.collection("users").document(userId)
                .collection("meditation").document(date).get().await().data?.values?.first() as Long?)?.toInt() ?: 0
            ...
            firestoreRef.collection("users").document(userId)
                .collection("meditation").document(date).set(mapOf("seconds" to (totalMeditationTime+seconds))).await()
        }
    }
